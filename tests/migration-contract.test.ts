...
test("initial migration persists the current safety and integrity invariants", () => {
  ...
  assert.match(migration, /scientific_references_prevent_delete/);
  assert.match(migration, /scientific reference is still used by a goal/);
  ...
  assert.match(migration, /private preference food ownership mismatch/);
  assert.match(migration, /food owner change would violate private ownership/);
  assert.match(migration, /goal interval overlap/);
  assert.match(migration, /julianday\(applied_at\) >= julianday\(confirmed_at\)/);
  assert.match(migration, /json_valid\(payload_json\) = 1/);
  assert.match(migration, /source_provider = 'manual-verified' OR \(source_external_id IS NOT NULL AND length\(trim\(source_external_id\)\) > 0\)/);
});
