...
3. **Deterministic numeric truth:** calories, macros, water totals, remaining targets, trends and adherence are calculated in code from structured verified data. AI must not author these totals, including by hiding numeric claims inside narrative strings.
...
7. **Health boundary:** ARVEN is not diagnostic or treatment software. It does not store or track medications. AI must not diagnose, prescribe, recommend medication use/changes or turn OCR/photo estimates into facts. Nutrition-related supplements remain a separate future module and must not be treated as medication management or medical dosing.
8. **Safety constraints are hard blocks:** allergies and explicit dietary safety exclusions override recommendation quality or convenience. Unresolved safety exclusions are not silently ignored; recommendation services must retrieve them from authenticated user context and fail closed until resolved.
9. **User isolation:** all user-owned persistence operations are scoped server-side by authenticated internal user id. Never trust a client-supplied owner id, and never allow a meal/log row to reference another user's private food.
...
13. **Real food imagery only:** a recipe/meal card may use an exact source photo with provenance/license metadata or the user's own photo. Never present a stock, representative or AI-generated image as the real photo of a specific recommended dish. If no exact real photo exists, show an intentional no-photo state.
14. **Change discipline:** develop on feature branches and review through pull requests; keep `main` releasable.
