...
## Progress and reports (measurements, body photos, milestones, PDF exports)

`body_measurements` (`db/migrations/0009_phase8_progress.sql`) is one row per logged moment — weight, body-fat percent, waist/hip/chest, and a free-text note, every figure independently nullable but at least one required (enforced by a table-level `CHECK`, not just Zod, matching this codebase's usual belt-and-suspenders precedent). It is a plain log, not a goal tracker: this app's `goal_versions` targets are macro/energy numbers (Phase 1), never a target body weight, so there is nothing here for a measurement to be "compared against" beyond its own history.

`body_photo_sets` gets its own table rather than a new `photo_assets.kind` — SQLite cannot widen an existing `CHECK` constraint without rebuilding the table, and `photo_assets` already has three phases of rows depending on its current shape. The upload path (`lib/progress/body-photo-upload.ts`) otherwise mirrors `lib/api/vision-upload.ts` exactly: bytes go through `lib/media/storage.ts`, only metadata (plus an optional `front`/`side`/`back` angle) lives in SQL.

`progress_milestones` is a deterministic achievement ledger — "milestones and achievements" in the roadmap, computed purely from a subject's own `body_measurements` history (`evaluateBodyMeasurementMilestones` in `v1-boundary.ts`): a count threshold (1/5/20 measurements logged) and a weight-change-observed threshold (1kg/5kg since the first weigh-in). Nothing here is AI-authored or goal-relative. A `(user_subject, milestone_key)` pair can only be earned once, enforced by both the service (checks `hasProgressMilestone` before inserting) and a unique index (defense in depth, unlike the race-prone `insertProposalIfAbsent` pattern — a single Durable Object instance processes one request at a time, so this is not a true concurrency race, just cheap insurance). A milestone is never retracted, even if the measurement that earned it is later deleted: it is a record of a moment, not a live-recomputed property.

"Daily/weekly reports" and "charts... and deterministic adherence" reuse existing deterministic sources rather than inventing new ones: `lib/progress/reports.ts`'s `buildDailyProgressReport` is `V1NutritionReadRepository.getDailySnapshot` (the same aggregate `Bugün` renders) plus that day's measurement if one exists, and `buildWeeklyProgressReport` is literally Phase 4's `computeWeeklyMetrics` (the same numbers `WeeklyInsightV1`'s narrative is grounded in) plus the week's weight change. Nothing new is computed twice; Phase 8 only adds a body-measurement dimension alongside nutrition adherence that already existed.

"PDF/shareable report generation" is `lib/progress/pdf.ts`: a small, dependency-free PDF writer using only the standard non-embedded Helvetica base-14 font (so no font file has to ship with the document), rather than pulling in a full PDF library for what is structurally one page of text lines. That font's encoding cannot represent five Turkish letters (ş/Ş/ğ/Ğ/ı/İ); the writer transliterates only those before layout and leaves every other Turkish letter (ç, ö, ü and capitals) untouched, since those are native to the encoding. `/api/progress/report` is the only place that ever touches the generated bytes: it renders the PDF, stores it via the same private `lib/media/storage.ts` object storage as photos/lab documents, and records a `progress_report_exports` metadata row — the same "metadata in SQL, bytes elsewhere" split as every other media type in this codebase.

Optional activity/sleep/step context integrations named in the Phase 8 roadmap entry are **explicitly not built**: they need a real external OAuth connection (Apple Health, Google Fit, or similar) that this delivery has no credentials or approved provider contract for, so any storage or sync logic written against an unverified third-party shape now would just be guessing. This is the same kind of deliberate, disclosed scope-down as Phase 6's "no medication registry" and Phase 7's "not a push-notification scheduler."

`/api/measurements`, `/api/measurements/[id]`, `/api/body-photos`, `/api/body-photos/[id]`, `/api/progress/milestones`, `/api/progress/summary`, `/api/progress/report` and `/api/progress/report/[id]` back the pre-existing `/gelisim` canonical placeholder page (Phase 1), now filled in with real data — again, no sixth bottom-navigation tab. Account deletion additionally purges `body_measurements`, `body_photo_sets`, `progress_milestones` and `progress_report_exports`; deleting body photos' and report PDFs' underlying bytes is handled by `/api/account` — see the Phase 9 section below.

## Portability, account deletion and hardening (Phase 9)

`lib/portability/export.ts`'s `buildUserExport` assembles the versioned JSON backup described in `docs/PORTABILITY.md`: the ten sections declared in `lib/portability/types.ts` (`ArvenNutritionExportV1`) — profile, the current goal, the timezone/nutrition-day preferences actually persisted today, full-history meal-log and water-log events, measurements, recipes, custom foods, ARVEN memory, and a media manifest (metadata only, never bytes). Two read paths needed for a full-history export did not exist before this phase — `V1Transaction.listNutritionEvents` (every event ever recorded, not just one local day) and `listCustomFoodVersions` (every custom food a subject owns) — both added alongside every other read here, scoped strictly to the authenticated subject. `/api/export` (GET) returns the backup as a downloadable file; `/api/export/csv` returns one of three human-readable tables (meal-log, water-log, measurements) per `docs/PORTABILITY.md`'s CSV layer — an interchange/report format, explicitly not re-importable.

`lib/portability/import.ts`'s `importUserExport` restores a previously downloaded backup, following the import principles `docs/PORTABILITY.md` lays out: the manifest/format is validated first (`validateExportManifest`), nothing in the uploaded file is ever trusted as an owner id (every restored row is always written under the server-authenticated subject), and every row gets a freshly generated id/`createdAt` rather than reusing the file's — the explicit, disclosed trade-off being that importing the same backup twice duplicates history instead of silently colliding or overwriting, which the returned per-section counts make visible to the caller. `media-manifest` rows are always reported as skipped on import: a backup never contains bytes, so there is nothing to re-attach them to.

Account deletion (`/api/account`, `DELETE`) closes the gap `purgeAuthenticatedUser`'s own doc comment flagged as "Phase 9 scope": every private-object byte this subject owns — meal/menu/product photos, lab documents, body-progress photos, generated PDF reports — is deleted first, then the account and every other row is purged in one transaction, the same media-then-metadata ordering `/api/photos/[id]` DELETE already used. The route requires an exact `{"confirm":"HESABIMI SIL"}` body rather than a bare DELETE, so a stray request can never trigger it. The UI for all of this — full backup download, per-section CSV download, restore-from-backup, and the account-deletion danger zone — lives at `/ayarlar/veri-ve-gizlilik`.

"Offline/error/loading states" gained three route-segment files that did not exist anywhere in the app before: `app/(app)/error.tsx` (catches a render/data error without a blank white page), `app/(app)/loading.tsx` (shown automatically while a segment's data is resolving), and `app/(app)/not-found.tsx`; `app/global-error.tsx` is the last-resort boundary for the rare case the root layout itself fails. `app/offline` (Phase 1) already covered the no-network case.

"Accessibility... QA" added the site's first keyboard-focus indicator (`:focus-visible` in `globals.css` — there was none before), a skip-to-content link in `AppShell`, and a `prefers-reduced-motion` rule. A literal manual iPhone/VoiceOver device pass is **not** part of this delivery — no physical device or screen-reader session is available in this environment — the same kind of disclosed, environment-driven scope-down as Phase 8's activity/sleep/step integrations.

"Cost telemetry, caching and request deduplication" is `lib/ai/telemetry.ts`, wired into the one external paid dependency this app has — `lib/ai/provider.ts`'s `performJsonCompletion`, the single choke point all six AI calls already went through. `recordAiUsage`/`getAiUsageSummary` (surfaced at `/api/telemetry/ai-usage`) track token counts and an approximate — explicitly not billed — cost per endpoint, in-process only, matching the "no metrics pipeline exists yet" reality documented elsewhere in this file. `dedupeInFlight` collapses concurrent identical requests (same model + messages, e.g. a double-tap send before the first reply arrives) into one network call; the cache entry is removed the instant the call settles, so it can never serve a stale answer to a later, genuinely new request — deliberately deduplication, not a result cache.

"Performance/security review": every new route added this phase reuses `resolveRouteContext` (never re-derives the authenticated subject), every export/import read/write is scoped by that subject exactly like the rest of the app, and no new route accepts a caller-supplied owner id anywhere (`tests/portability-route-regressions.test.ts` pins this at the source level, the same pattern `phase6-boundary-regressions.test.ts` established). No new database indexes were needed: every new read (`listNutritionEvents`, `listCustomFoodVersions`) filters on `user_subject`/`owner_subject`, already indexed by the Phase 1 baseline schema.
...
The Phase 9 (portability, account deletion and hardening) change set was verified locally end-to-end before merge: the full test suite (272 tests), the TypeScript type check, the migration validation script, and the production build all pass cleanly on Node 22.
