...
const HouseholdSelection = z.object({ kind:z.literal("household"), portionVersionId:Id, quantity:z.number().finite().positive().max(20) }).strict();
...
export interface V1Transaction {
  ...
  getFoodVersion(userSubject:string,foodVersionId:string):Promise<Food|null>;
...
function canonicalJson(value:unknown):string{if(value===null||typeof value!=="object")return JSON.stringify(value);if(Array.isArray(value))return `[${value.map(canonicalJson).join(",")}]`;const r=value as Record<string,unknown>;return `{${Object.keys(r).sort().map(k=>`${JSON.stringify(k)}:${canonicalJson(r[k])}`).join(",")}}`;}
