...
def main():
    ...
    # Identity and REPLACE collision invariants.
    ...
    # Goal parent and calculator identity invariants.
    ...
    """, (NOW,))
    conn.execute("INSERT INTO goal_meal_allocations(goal_id,allocations_json,updated_at) VALUES('g1','[{"mealType":"lunch","energyShareBps":10000}]',?)", (NOW,))
    reject(conn, "allocation parent transfer", "UPDATE goal_meal_allocations SET goal_id='g2' WHERE goal_id='g1'")

    # Food truth/versioning and nutrient parent invariants.
    ...
    # REPLACE cannot bypass immutable meal history.
    ...
    # Natural claims require verified portion evidence.
    ...
    # Active proposal freezes core food truth before user confirmation.
    meal_payload = '{"localDate":"2026-09-02","occurredAt":"2026-09-02T20:00:00Z","mealType":"lunch","items":[{"foodId":"food-a","grams":50,"calculationVersion":"v1"}]}'
    conn.execute("""
    ...
    # Authenticated allergy safety is mandatory at proposal creation.
    ...
    milk_payload = '{"localDate":"2026-09-02","occurredAt":"2026-09-02T20:00:00Z","mealType":"lunch","items":[{"foodId":"milk-food","grams":50,"calculationVersion":"v1"}]}'
    reject(conn, "allergen-conflicting proposal", """
      INSERT INTO ai_actions(id,user_id,action_type,schema_version,request_hash,payload_json,status,idempotency_key,created_at)
      VALUES('unsafe','u1','meal-log','MealLogActionV1','h2',?,'proposed','unsafe',?)
    """, (milk_payload, NOW))

    # Applied actions require exactly one matching persisted mutation.
    water_payload = '{"localDate":"2026-09-02","occurredAt":"2026-09-02T20:00:00Z","milliliters":250}'
    conn.execute("""
      INSERT INTO ai_actions(id,user_id,action_type,schema_version,request_hash,payload_json,status,idempotency_key,created_at)
    ...
    conn.execute("INSERT INTO water_logs(id,user_id,occurred_at,local_date,milliliters,created_at,ai_action_id) VALUES('w1','u1',?,'2026-09-02',250,?,'water-action')", (NOW, NOW))
    conn.execute("UPDATE ai_actions SET status='applied',applied_at=? WHERE id='water-action'", (LATER,))
    reject(conn, "second persisted result for action", "INSERT INTO water_logs(id,user_id,occurred_at,local_date,milliliters,created_at,ai_action_id) VALUES('w2','u1',?,'2026-09-02',250,?,'water-action')", (NOW, NOW))
    reject(conn, "delete applied mutation receipt", "DELETE FROM water_logs WHERE id='w1'")

    # Failure audit cannot invent confirmation evidence.
    conn.execute("""
      INSERT INTO ai_actions(id,user_id,action_type,schema_version,request_hash,payload_json,status,idempotency_key,created_at)
